<?php

class NetUtils
{

    const CMDCHUNK=64;
    const PORT_START=20000;
    const PORT_END=65534;
    const IMGCHUNK=1450;
    const broker_port=5020;
    private $broker_address;
    private $sock;
    private $my_address;
    private $my_port;
    private $my_username;
    private $my_uuid;
    
    public $err_img;
    
    public function __construct($username, $uuid, $port)
    {
        if (!isset($uuid)){
            throw new Exception(" Invalid uuid ".$uuid);
        }
        $this->my_username = $username;
        $this->my_uuid = $uuid;
        $this->broker_address = gethostbyname('broker.ibeyonde.com');
        //error_log("Broker address=".$this->broker_address);
        if (isset($_SERVER['SERVER_ADDR'])) {
            $this->my_address = $_SERVER['SERVER_ADDR'];
            $this->err_img = file_get_contents($_SERVER["DOCUMENT_ROOT"] ."/img/no_update_error.png");
        }
        else {
            $this->my_address = gethostbyname('app.ibeyonde.com');
            $this->err_img = "";
        }
        
        //$bignum = hexdec( substr(md5($username.$uuid), 0, 15) );
        //$smallnum = $bignum % 45534;
        //$this->my_port = abs($smallnum) + NetUtils::PORT_START;
        //error_log("Port=".$this->my_port);
        $this->my_port = $port;
        //error_log("Port=".$this->my_port);
        
    }
    
    public function addr2bytes($address, $port){
        return pack('Nv', ip2long($address), intval($port));
    }
    
    public function bytes2address($data){
        $array = unpack('N1address/v1port', $data);
        return array(long2ip($array['address']), intval($array['port']) );
    }
    
    public function getSocket(){
        if ($this->sock != null){
            socket_close($this->sock);
        }
        if (($this->sock = socket_create(AF_INET, SOCK_DGRAM, SOL_UDP)) === false) {
            throw new Exception("socket_create() failed: reason: " . socket_strerror(socket_last_error()));
        }
        
        if (socket_set_option($this->sock, SOL_SOCKET, SO_REUSEADDR, 1) == false) {
            throw new Exception("socket_set_option() failed: reason: " . socket_strerror(socket_last_error($this->sock)));
        }
        
        if (socket_bind($this->sock, 0, intval($this->my_port)) === false) {
            throw new Exception("socket_bind() failed: reason: " . socket_strerror(socket_last_error($this->sock)));
        }
        socket_set_option($this->sock, SOL_SOCKET, SO_RCVTIMEO, array('sec' => 4, 'usec' => 0));
        socket_set_option($this->sock, SOL_SOCKET, SO_SNDTIMEO, array('sec' => 2, 'usec' => 0));
    }
    
    public function register(){
        $this->getSocket();
        //error_log("NETUTILS: Registering ". substr($this->my_username.$this->my_uuid, 0, 30));
        $this->sendCommandBroker("REGISTER:". substr($this->my_username.$this->my_uuid, 0, 30) . ":".$this->addr2bytes($this->my_address, $this->my_port));
        list($cmd, $data) = $this->recvCommandBroker();
        list($address, $port) = $this->bytes2address($data);
        //error_log('NETUTILS: Register address='.$address.' and port='.$port.' cmd='.$cmd);
    }
    //Broker-----------------RPADDR: uuid =d97080b2 peeraddr=183.82.43.69 and peerport=32543, 
    // Peer-----------------PADDR: uuid =d97080b2 peeraddr=100.57.55.48 and peerport=12344, 
    public function getPeerAddress($uuid){
        $this->sendCommandBroker("PADDR:".$uuid.":");
        list ( $cmd, $data ) = $this->recvCommandBroker ();
        list ( $peeraddr, $peerport ) = $this->bytes2address ( $data );
        //error_log ( "Broker-----------------".$cmd.": uuid =" . $uuid . " peeraddr=" . $peeraddr . " and peerport=" . $peerport );
        list($cmd, $data) = $this->recvCommandPeer($peeraddr, $peerport);
        //error_log( "Peer-----------------".$cmd.": uuid =" . $uuid );
        return array( $peeraddr, $peerport );
    }

    public function initiate($quality){
        if ($quality == Quality::A){
           $this->sendCommandBroker(Quality::A.":".$this->my_uuid.":");
        }
        else if ($quality == Quality::B){
          $this->sendCommandBroker(Quality::B.":".$this->my_uuid.":");
        }
        else if ($quality == Quality::C) {
           $this->sendCommandBroker(Quality::C.":".$this->my_uuid.":");
        }
        else if ($quality == Quality::D) {
           $this->sendCommandBroker(Quality::D.":".$this->my_uuid.":");
        }
        else if ($quality == Quality::E) {
           $this->sendCommandBroker(Quality::E.":".$this->my_uuid.":");
        }
        else if ($quality == Quality::F) {
            $this->sendCommandBroker(Quality::F.":".$this->my_uuid.":");
        }
        else {
            $this->sendCommandBroker(Quality::G.":".$this->my_uuid.":");
        }
    }

    
    public function initiateV($quality){
        if (VQuality::qualityValueExists($quality)){
            $this->sendCommandBroker($quality.":".$this->my_uuid.":");
        }
        else {
            $this->sendCommandBroker(VQuality::N.":".$this->my_uuid.":");
        }
    }
    
    public function sendCommandBroker($cmdbuff){
        //error_log('sendCommandBroker Sending '.$cmdbuff." broker address".$this->broker_address. "broker port=".NetUtils::broker_port);
        $len = strlen($cmdbuff);
        if ($len > NetUtils::CMDCHUNK){
            error_log("NETUTILS: sendCommandBroker undesired size ".$len." buff=".$cmdbuff);
        }
        if (socket_sendto( $this->sock , $cmdbuff ,$len , MSG_EOF, $this->broker_address , NetUtils::broker_port ) == false){
            throw new Exception("socket_sendto() failed: reason: " . socket_strerror(socket_last_error($this->sock)));
        }
    }
    
    public function recvCommandBroker(){
        $buf='';
        $broker_port=null;
        if (socket_recvfrom($this->sock, $buf, NetUtils::CMDCHUNK, MSG_WAITALL, $this->broker_address, $broker_port) == false){
            throw new Exception("socket_recvfrom() failed: reason: " . socket_strerror(socket_last_error($this->sock)));
        }
        //error_log('NETUTILS: Receiving '.$buf);
        if (strpos($buf,":") !== false){
            return explode(":", $buf);
        }
        else {
            return array("NULL", "NULL");
        }
    }
    
    public function sendCommandPeer($peeraddr, $peerport, $cmd, $size=null){
        //error_log('sendCommandPeer Sending '.$cmd." broker address".$this->broker_address. "broker port=".NetUtils::broker_port);
        $cmdbuff=null;
        if ($size != null){
            $cmdbuff = $cmd.":".$this->my_uuid.".".$size;
        }
        else {
            $cmdbuff = $cmd.":".$this->my_uuid;
        }
        $len = strlen($cmdbuff);
        if ($len > NetUtils::CMDCHUNK){
            error_log("NETUTILS: sendCommandPeer undesired size ".$len." buff=".$cmdbuff);
        }
        if (socket_sendto( $this->sock , $cmdbuff , $len, MSG_EOF, $peeraddr, $peerport) == false){
            throw new Exception("socket_sendto() failed: reason: " . socket_strerror(socket_last_error($this->sock)));
        }
    }
    
    public function recvCommandPeer($peeraddr, $peerport){
        //error_log('NETUTILS: Start Receive from peer ');
        $buf='';
        if (socket_recvfrom($this->sock, $buf, NetUtils::CMDCHUNK, MSG_WAITALL, $peeraddr, $peerport) == false){
            throw new Exception("socket_recvfrom() failed: reason: " . socket_strerror(socket_last_error($this->sock)));
        }
        //error_log('NETUTILS: Receiving '.$buf);
        if (strpos($buf,":") !== false){
            return explode(":", $buf);
        }
        else {
            return array("NULL", "NULL");
        }
    }
    
    public function recvAllPeer($size, $peeraddr, $peerport){
        $img='';
        $buf='';
        $remaining = $size;
        while ($remaining > 0){
            $buf_len = socket_recvfrom($this->sock, $buf, $remaining, MSG_WAITALL, $peeraddr, $peerport);
            if ($buf_len  == false){
                throw new Exception("socket_recvfrom() failed: reason: " . socket_strerror(socket_last_error($this->sock)));
            }
            $img .= $buf;
            $remaining -= $buf_len;
        }
        //error_log('NETUTILS: Received All remaining '.$remaining);
        return $img;
    }
    
    public function sendAllPeer( $peeraddr, $peerport, $blob, $size){
        $remaining = $size;
        //error_log("sendAllPeer size =" . $size . " length = " . strlen($blob));
        while ($remaining > 0){
            $sent = socket_sendto($this->sock, $blob, NetUtils::IMGCHUNK, MSG_EOF, $peeraddr, $peerport);
            if ($sent  == false){
                throw new Exception("socket_sendto() failed: reason: " . socket_strerror(socket_last_error($this->sock)));
            }
            $blob = substr($blob, $sent);
            //error_log("sendAllPeer ".$sent);
            $remaining -= $sent;
        }
        //error_log('NETUTILS: Received All remaining '.$remaining);
    }
    
    public function sendActionBroker($uuid, $action, $data){
        //error_log('NETUTILS:sendActionBroker($uuid, $action, $data)');
        $this->getSocket();
        $this->sendCommandBroker("ACTION:".$uuid.":".base64_encode($action.":".$data));
    }
    
    public function getTimeDeltaForLastPing(){
        try {
            $this->sendCommandBroker("TIME:".$this->my_uuid .":");
            list($cmd, $seconds) = $this->recvCommandBroker();
            //error_log("$cmd Time is $seconds");
            if ($seconds == -1) {
                //error_log("$cmd Device offline");
                return PHP_INT_MAX ;
            }
            else {
                //error_log("$cmd Time delta is $seconds seconds");
                return $seconds;
            }
        }
        catch (Exception $e){
            error_log($e);
        }
        return 1;
    }
    
    public function close(){
        socket_close($this->sock);
    }
    
    
}

     
